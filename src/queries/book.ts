...
export const queryInsertBook = async (
  ...
  const promisePool = pool.promise();
  const [rows] = await promisePool.query<ResultSetHeader>(
    "INSERT INTO book (library_user, title, author, year, topic, isbn, home_office_id) VALUES (?)",
    [[userId, title, author, year, topic, isbn, homeOfficeId]]
  );
  return rows.affectedRows != 0;
};
...
export const queryUpdateBook = async (book: Book): Promise<boolean> => {
  const promisePool = pool.promise();
  const [rows] = await promisePool.query<ResultSetHeader>(
    "UPDATE book SET title=(?), author=(?), year=(?), topic=(?), isbn=(?), home_office_id=(?) WHERE id=(?)",
    [
      book.title,
      book.author,
      book.year,
