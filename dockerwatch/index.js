...
async function updateContainer(containerId) {
    ...
    const log = async (str) => {
        const date = new Date();
        console.log(
            `[\x1b[36m${date.toLocaleString()}\x1b[0m] \x1b[35m${repoTag} |\x1b[0m ${str}`
        );
        await fs.appendFile(
            `./log_${date.getFullYear()}_${date.getMonth()}_${date.getDate()}.txt`,
            `[${date.toLocaleString()}] ${repoTag} | ${str}\n`
        );
    };
                ...
                async (event) => {
                    ...
                    if (
                        ...
                        Object.values(status).forEach((s) => {
                            console.log(
                                `[\x1b[36m${new Date().toLocaleString()}\x1b[0m] \x1b[35m${repoTag} |\x1b[0m ${
                                    s.status
