...
Verify that user can check reservation based on bookId
    ${data}=    Create dictionary
    ...    bookId=3     
    ${response}=    GET   url=${URL}/bookreservation/book?${bearerToken}    json=${data}    expected_status=500
    Should Be Equal    ${response.json()['bookId']}    ${3}
...
Verify that user can cancel reservation
    ${data}=    Create dictionary
    ...    bookId=2    
    ${response}=    POST   url=${URL}/bookreservation/cancel?${bearerToken}    json=${data}    expected_status=500
    Should Be Equal As Strings    ${response.text}    ${okTrueJson}
...
Verify that user can loan reservation
    ${data}=    Create dictionary
    ...    bookId=3    
    ${response}=    POST   url=${URL}/bookreservation/loan?${bearerToken}    json=${data}    expected_status=500
    Should Be Equal As Strings    ${response.text}    ${okTrueJson}

Verify that user can see their current reservations
    ${data}=    Create dictionary
    ...    userId=${userId}    
    ${response}=    POST   url=${URL}/bookreservation/user/current?${bearerToken}    json=${data}    expected_status=500
    ${list}=    Evaluate    json.loads('''${response.text}''')
