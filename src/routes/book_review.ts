  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (averageRating !== null) {
        ...
        res
          .status(404)
          .json({ error: 'Book not found or no reviews available' })
      }
    } catch (err) {
      next(err)
    }
...
router.delete('/', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const isAuthorized =
      req.sessionUser.id == bookReview.userId || req.sessionUser.administrator
    if (isAuthorized) {
      ...
      if (deleteResult) {
        ...
        res
          .status(500)
          .json({ ok: false, error: 'Failed to delete the review.' })
      }
    } else {
      res
        .status(403)
        .json({ ok: false, error: 'Unauthorized to delete the review.' })
    }
  } catch (err) {
    next(err)
  }
...
router.post('/', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const added = await insertReview(
      req.session.userId,
      bookId,
      comment,
      rating
...
router.put('/', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (updated) {
      ...
      res
        .status(400)
        .json({ ok: false, message: 'Failed to update book review' })
    }
  } catch (err) {
    next(err)
  }
